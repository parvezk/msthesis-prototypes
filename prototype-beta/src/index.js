...
function showPredictions(predictions) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    children[i].children[0].setAttribute(
      "style",
      "width: " + Math.round(prob * 100).toString() + "%"
    );
...
function setupListeners() {
  document.querySelector("#model-selector").addEventListener("change", () => {
...
async function AttributionMatrix(img_tensor) {
  ...
  model.layers.map(layerName => {
    if (layerName.name.startsWith("block", 0)) layerNames.push(layerName.name);
  });

  const layerOutputs = layerNames.map(
    layerName => model.getLayer(layerName).output
  );
  const activation_model = tf.model({
    inputs: model.input,
  ...
  for (let i = 0; i < activations.length - 1; ++i) {
    ...
    let activationTensors = tf.split(
      activations[i],
      activations[i].shape[activations[i].shape.length - 1],
      -1
    );
...
async function FeatureMaps(image) {
  ...
  for (let i = 0; i < 8; ++i) {
    ...
    dagG.selectAll(".fv-ch-" + layerName).each(async function(d, j) {
      const container = d3.select(this).node();
      var xhtmlNS = "http://www.w3.org/1999/xhtml";
      var context = container
        .getElementsByTagNameNS(xhtmlNS, "canvas")[0]
        .getContext("2d");
...
async function ActivationGraph(image) {
  ...
  for (let i = 0; i < activations.length - 1; ++i) {
    ...
    compression = compression.reshape([
      compression.shape[1],
      compression.shape[2],
      compression.shape[3]
    ]);
